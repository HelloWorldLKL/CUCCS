                  ...
                  <el-row class="control shadow">
                    ...
                    <el-col :span="9">
                      <el-dropdown @command="downloadDocument">
                        ...
                        <el-dropdown-menu slot="dropdown">
                          <a :href="`http://case.minipoint.cn/cucc/${section.sDocPath}`" target="_blank"><el-dropdown-item><i class="fa fa-file-word-o"></i>&nbsp;DOC 文档</el-dropdown-item></a>
                          <a :href="`http://case.minipoint.cn/cucc/${section.sPptPath}`" target="_blank"><el-dropdown-item divided><i class="fa fa-file-powerpoint-o"></i>&nbsp;PPT 文档</el-dropdown-item></a>
                        </el-dropdown-menu>
                      </el-dropdown>
                    </el-col>
...
export default {
  ...
  methods: {
    changePDFedition(command) {
      ...
      if (command === 'pdf') {
        this.fileName = this.section.sPptPdfPath
      }
      if (command === 'doc') {
        this.fileName = this.section.sDocPdfPath
      }
    },
  ...
  created() {
    if (util.getQueryString('sID')) {
      this.$http.get(`http://localhost:3000/api/section?sID=${util.getQueryString('sID')}`).then((response) => {
