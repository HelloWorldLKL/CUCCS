...
export default {
  ...
  methods: {
    handleCurrentChange(val) {
      ...
      this.$http.get(`http://localhost:3000/api/getSourceByCourseID?cID=${util.getQueryString('cID')}&cPage=${this.currentPage}&pSize=${this.pageSize}`).then((response) => {
  ...
  created() {
    ...
    this.$http.get(`http://localhost:3000/api/getCourseByCourseID?cID=${util.getQueryString('cID')}`).then((response) => {
